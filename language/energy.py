...
class MinimizeCRmsd(EnergyTerm):
    ...
    def compute(self, node, folding_result: FoldingResult) -> float:
        ...
        # Create a mask to store which atoms in 'atoms' match with 'template'
        # mask = np.zeros(len(atoms), dtype=bool)
        
        # template_index = self.template.res_id[0]
        # # print(self.template)
        # # print(atoms)
        # # print(atoms[1].__getattr__("res_id"))
        # mask = np.zeros(len(atoms), dtype=bool)
        # # Go through each atom in 'template'
        # for i in range(len(self.template)):
        #     # Find matches in 'atoms'
        #     # matches = (atoms[i].__getattr__("res_id") == self.template[i].__getattr__("res_id") -template_index+1) & (atoms[i].__getattr__("res_name") == self.template[i].__getattr__("res_name"))
        #     num_matches = atoms[i].__getattr__("res_id") == (self.template[i].__getattr__("res_id")-template_index+1) 
        #     name_matches = atoms[i].__getattr__("res_name") == self.template[i].__getattr__("res_name")
        #     # If a match is found, keep the atom in 'atoms'
        #     if np.all([num_matches, name_matches]):
        #          mask[i] = True  
        #     else:
        #         print([num_matches, name_matches])
        #         print(atoms[i].__getattr__("res_name"))
        #         print(atoms[i].__getattr__("res_id"))
...
class MinimizeTargetDistance(EnergyTerm):
    def __init__(self, target: AtomArray, backbone_only: bool = False) -> None:
        super().__init__()

        self.target: AtomArray = target
        self.backbone_only: bool = backbone_only
        if self.backbone_only:
            self.target = get_backbone_atoms(target)
    ...
    def compute(self, node, folding_result: FoldingResult) -> float:
        ...
        return carbon_alpha_distance(self.target, atoms)


def carbon_alpha_distance(atom_array_a: AtomArray, atom_array_b: AtomArray) -> float:
    """
    This function computes the mean Carbon Alpha distance between two AtomArrays.
    """

    # Filter for Carbon Alpha atoms
    ca_atoms_a = atom_array_a[atom_array_a.atom_name == "CA"]
    ca_atoms_b = atom_array_b[atom_array_b.atom_name == "CA"]


    # Compute pairwise distances between all pairs of Carbon Alpha atoms across the two arrays
    pairwise_dists = np.linalg.norm(ca_atoms_a.coord[:, np.newaxis, :] - ca_atoms_b.coord[np.newaxis, :, :], axis=-1)
    
    # Sort the distances
    sorted_dists = np.sort(pairwise_dists, axis=None)
    
    # Determine the index that cuts off the lowest 30% of distances
    cutoff_index = int(0.5 * sorted_dists.size)
    
    # Return the mean of the pairwise distances
    # return pairwise_dists.mean()
    # Return the mean of the smallest 30% distances
    return sorted_dists[:cutoff_index].mean()